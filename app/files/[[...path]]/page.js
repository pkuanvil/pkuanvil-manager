...
export default async function Files_Page({ params }) {
	...
	try {
		...
		const data = await b2_list_file_names(conf, auth, prefixFull);
		return (
			<>
				<Header></Header>
				<NavSide currentPath={'/files'}></NavSide>
				<main className="lg:pl-80">
					<Files initFiles={data.files} prefixFull={prefixFull} authToken={auth.authorizationToken} downloadPrefix={downloadPrefix}></Files>
				</main>
			</>
		)
	} catch (e) {
